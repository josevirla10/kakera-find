...
export function ProductModal({ product, onClose }: Props) {
  ...
  async function handleShare() {
    if (navigator.share) {
      try {
        await navigator.share({ title: product.title, url: product.permalink })
      } catch { /* cancelled */ }
    }
  }
  ...
  const whatsappUrl = `https://wa.me/?text=${encodeURIComponent(`${product.title} — ${product.permalink}`)}`
      ...
      <div className="relative w-full sm:max-w-lg bg-surface rounded-t-2xl sm:rounded-2xl shadow-2xl overflow-hidden max-h-[92dvh] flex flex-col">
        ...
        <div className="flex flex-col gap-4 p-5 overflow-y-auto">
          ...
          <div className="flex flex-col gap-2">
            <p className="text-xs font-semibold text-content-tertiary uppercase tracking-wide">Compartir</p>
            <div className="flex gap-2">
              {/* Web Share API — solo aparece si el browser lo soporta */}
              ...
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="flex-1 flex items-center justify-center gap-2 py-2.5 px-3 border border-border-subtle rounded-xl text-sm text-content-secondary hover:text-green-600 hover:border-green-400 transition-colors"
              >
